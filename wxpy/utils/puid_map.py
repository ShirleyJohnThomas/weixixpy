...
"""
#puid
尝试用聊天对象已知的属性，来查找对应的持久固定并且唯一的用户ID
##数据结构
PuidMap 中包含4个dict,分别为
1. user_name -> puid
2. wxid -> puid
3. remark_name ->puid
...
## 查询逻辑
当给定一个Chat对象，需要获取对应的puid时，将按顺序，使用自己的对应属性，轮询以上4个dict
* 若匹配任何一个，则获取到puid，并将其他属性更新到其他的dict
*如果没有匹配，则创建一个新的puid,并加入到以上的4个dict
"""

class PuidMap(object):
    def __init__(self,path):
        """
            用于获取聊天对象的puid(持续有效并且稳定唯一的用户ID),和保存映射关系
    ...
    def get_puid(self,chat):
        ...
        with self._thread_lock:
            if not (chat.user_name and chat.user_name):
                return
...
class TwoWayDict(UserDict):
    """
        可双向查询，且key,value均为唯一的dict
        限制:key,value均须为不可变对象，且不支持.update()方法
    """
    def __init__(self):
        if PY2:
